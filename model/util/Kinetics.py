...
class Kinetics(torch.utils.data.Dataset):
    ...
    def construct_loader(self):
        ...
        path_to_file = os.path.join(self.path_to_data_dir[0],"{}.list".format(file_name[self.mode]))
        ...
        with open(path_to_file, "r") as f:
            for path_label in f.read().splitlines():
                ...
                self.labels.append(int(label))  # 读取完整标签文件
    ...
    def __getitem__(self, index):
        ...
        if self.mode in ["pretrain"]:
            ...
            return video, torch.tensor(self.labels[index])
